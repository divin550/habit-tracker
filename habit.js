...
function renderDays(habitId) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        dayElement.innerHTML = `
          ...
          <button class="status-btn ${color}" data-date="${dateStr}" ${isPast ? 'disabled' : ''}>
...
function toggleStatus(habitId, dateStr) {
    ...
    if (clickedDate < today) {
        alert('You can only update today or future days. Past entries are locked to prevent progress cheating.');
        return;
    }
...
window.onload = init;let currentWeekStart = new Date();
let notificationSettings = {
    enabled: false,
    time: "09:00",
    habits: []
